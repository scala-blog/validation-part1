...
object Example6HandlingTheUnexpected:
  ...
  object SSN:
    def fromString(string: String): Either[String, SSN] =
      Try {
        ...
        case Failure(exception) => Left(s"ERROR: Unexpected: parsing ssn $string: " + exception.getMessage)

    private def fromStringUnsafe(string: String): Either[String, SSN] =
      if (string == null)
  ...
  object Age:
    def fromString(string: String): Either[String, Age] =
      Try {
        ...
        case Success(good) => good
        case Failure(exception) => Left(s"ERROR: Unexpected: parsing age '${string}': " + exception.getMessage)

    private def fromStringUnsafe(string: String): Either[String, Age] =
      if (string == null)
        Left("Age is null")
